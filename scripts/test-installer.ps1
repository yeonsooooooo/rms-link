...
& ./collect-logs.ps1 -OutputDirectory "$pwd/.work/support-before-install"
if (-not (Get-ChildItem '.work/support-before-install/*.zip')) { throw 'Pre-install diagnostic bundle missing' }
...
if (Get-ItemPropertyValue 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Run' -Name RmsLink -ErrorAction SilentlyContinue) { throw 'Reinstall reset automatic-start preference' }
...
& ./collect-logs.ps1 -OutputDirectory "$pwd/.work/support-after-failure"
...
@{passed=$true;version=$version;install=$true;setupWindow=$true;reinstallPreservesSettings=$true;failedRuntimePreservesVersion=$true;standaloneDiagnostics=$true;physicalKeytechVerified=$false} | ConvertTo-Json | Set-Content 'installer-fixture-validation.json'
